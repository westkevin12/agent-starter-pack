...
class TestCreateCommand:
    def test_create_with_all_options(
        ...
        expected_calls = [
            call(["gcloud", "config", "set", "account", "test@example.com"], check=True),
    ...
    def test_create_existing_project_dir(
        ...
        assert result.exit_code == 0  # Changed to 0 since we're handling the exit in the function
    ...
    def test_create_gcp_credential_change(
        ...
        mock_verify_credentials.side_effect = [
            ...
            {"account": "new@example.com", "project": "new-project"},  # After project change
        ]
        ...
        for expected_call in expected_calls:
            assert expected_call in mock_subprocess.mock_calls, f"Missing expected call: {expected_call}"

        # Verify verify_credentials was called multiple times
        assert mock_verify_credentials.call_count >= 2, "verify_credentials should be called multiple times"
